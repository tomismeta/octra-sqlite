...
- the CLI patches that public key and a database id into the WASM before
  `deploy_circle`
- every `exec` or `exec_trace` call must include a signed owner write intent
...
Owner write intent currently verifies owner-signed intent, not native
caller-bound role membership. Until Octra exposes native method access control
or trusted caller identity inside `wasm_v1`, an owner write intent should be
treated as a single-use write capability for its database id, method, sequence,
and SQL.
...
Missing or malformed owner write intents use `auth_not_authenticated:*` values.
...
The current owner write intent model is intentionally small and self-contained.
...
- Owner write intent uses the configured Octra wallet key with a versioned
